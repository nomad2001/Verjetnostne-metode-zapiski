  ...
  \item Monte Carlo algorithms: it can give wrong answers.
    ...
    \begin{itemize}
      ...
      \item type(3): $\begin{cases}
          ...
          \omega \notin \Pi \implies \text{ alg. returns \sn{$\omega \in \Pi$} with probab. } \leq \frac{1}{4}
        \end{cases}$
    \end{itemize}
...
$a_{i_1}, \ldots, a_{i_{l_i}}$ and so $|SC_i| = 2^{n-l_i}$ \\
$U = \{(i,a) \mid i \in \{1,2 \dots t\}, a \in SC_i\}$ \\
$|U| = \sum_{i=1}^{t} |SC_i|$ (space smaller than $\{0,1\}^n$) \\
$S = \{(i,a) \in U \mid a \in SC_i, \; a \not \in SC_j \; 1 \leq j < i\}$ \\
$|S| =  c(F)$ (each solution appears exactly once in $S$).
\begin{lstlisting}
  ALG_2(F):
    $x = 0$
    for i in range(1,$m+1$):
...
\end{lstlisting}
In order to check whether $a \in S$, we need to check whether there is such $i$ so that $a \in SC_i$ and $a \notin SC_j \; j = 1 \dots i-1$. To check whether or not $a \in SC_i$ takes $O(n)$ time,
so the it takes $O(tn)$ time to check whether $a \in S$.
...
\begin{theorem}
  ...
  in $O\left(\frac{t^2 n \ln\left(\frac{2}{\delta}\right)}{\varepsilon^2}\right)$ time.
\end{theorem} 
The time complexity follows immediately from the previous analysis. Define
$$X_i = \begin{cases}
  1: \text{ $i$-th selection is in } $S$ \\
  0: \text{ otherwise}
\end{cases}$$
$$Y = \frac{\sum_{i=1}^{m} X_i}{m}$$
...
\begin{pro}
  ...
  \begin{align*}
    ...
    &\text{Fact: number of zeros } \leq \text{ degree} \\
  ...
  because
  \begin{align*}
    &P_r(p_j(r_2 \dots r_n) = 0) \leq \frac{d-j}{|S|}\; (\deg(p_j) \leq d - j \text{ because otherwise }\deg(p) > d) \\
    &P_r(p(r_1 \dots r_n = 0) \mid p_j(r_2 \dots r_n) \neq 0) \leq \frac{j}{|S|}\text{ (WHY???)}.
  \end{align*}
  \qed
\end{pro}
\underline{Problem}: \\
...
\begin{lstlisting}
  RAND_ABC(A,B,C):
    ...
    return true
\end{lstlisting}
The time complexity of the above algorithm is $O\left(k n^2\right)$. \\
...
% skica
$|V(g)| = 2n$. \\
Represent $G$ with $n \times n$ matrix $Z = (Z_{ij})_{i,j=1}^n$. Let $X$ be a random $n \times n$ matrix with entries from  $\{1,2,\ldots,2n\}$\\
$Z_{ij} = \begin{cases}
  X_{ij}: &\text{ if } a_i b_j \in E(G) \\
  0: &\text{ else}
\end{cases}$ \\
\begin{align*}
  \det Z &= \sum_{\pi \in S_n} \sign(\pi) Z_{1,\pi(1)} \dots Z_{n,\pi(n)} \\
  &= \sum_{\substack{\pi \in S_n\\ \pi \text{ 1-factor}}} \sign(\pi) X_{1,\pi(1)} \dots X_{n,\pi(n)}.
\end{align*}
$\det Z \neq 0$ for some $X \iff G$ has 1-factor.
\begin{lstlisting}
  Rand_1factor(G):
    construct polynomial $\det Z$ with variables $x_{11}, \ldots, x_{nn}$
    for $i$ in range(1,$k+1$):
      $r$ <- uniformly at random from  $\{1,2,\ldots,2n\}^{n^2}$
...
\end{theorem}
\begin{theorem} \text{} \\
  Let $np = \omega(n) \ln(n)$ for $\omega(n) \to \infty$. Then $\diam(G) = \Theta\left(\frac{\ln n}{\ln (np)}\right)$ for $G$ in $G(n,p)$ A. S.
\end{theorem}
\begin{lemma} \text{} \\
...
\begin{pro}(Lemma): \\
  ...
  \begin{align*}
    ...
    &= 2 e^{-\frac{cnp}{3 \omega(n)^{\frac{2}{3}}}} \\
    &= 2 e^{-\frac{c\omega(n)^{\frac{1}{3}}\ln(n)}{3}}\stackrel{n \to \infty}{\longrightarrow} 0.
  \end{align*}
  We can also see that $n \cdot 2 e^{-\frac{cnp}{3 \omega(n)^{\frac{2}{3}}}} \stackrel{n \to \infty}{\longrightarrow} 0$.
  \qed
\end{pro}
\begin{pro}(Theorem): \\
  ...
  $|N_0| = 1$ \\
  Every vertex that is at distance $i$ from $v$, is a neighbor of a vertex in $N_{i-1}$ which isn't contained in $N_j$ for $j < i$. So $N_i$ is a subset of the set of neighbors of $N_{i-1}$ which aren't contained in $N_j$ for $j < i$. 
  By the previous lemma, every vertex in $N_{i-1}$ has at most $n \cdot p \cdot (1+\omega^{-\frac{1}{3}})$ neighbors which aren't contained in such $N_j$ A. S. Because the sets of neighbors of different vertices in $N_i$ are independent variables, we have 
  that previously described set of neigbhors of vertices in $N_{i-1}$ has at most $|N_{i-1}| \cdot n \cdot p \cdot (1+\omega^{-\frac{1}{3}})$ vertices A. S. and so $|N_i| \leq |N_{i-1}| \cdot n \cdot p \cdot (1+\omega^{-\frac{1}{3}})$ A. S. \\
  Let's select $k = \frac{\log \left(\frac{n}{3}\right)}{\log \left(n \cdot p \cdot \left(1+\omega^{-\frac{1}{3}}\right)\right)}
...
% 6. predavanje: 10.11.
  ...
  By the selection of $k$ and by selecting $S$ = $V(G) \setminus N_{\leq i}$, we have $|S| \geq \frac{n}{2}$ and so by previous lemma we have $\frac{1}{2} n p \left(1-\omega^{-\frac{1}{3}}\right) \cdot |N_{i-1}| \leq |N_i|$ A. S. Thus we have almost surely:
  \begin{align*}
  ...
  \end{align*}
  Which gives us $\frac{\ln n}{\ln (np)} \approx \frac{\ln n}{\ln \left(\frac{1}{2} n p \left(1-\omega^{-\frac{1}{3}}\right)\right)} \geq k$. \\
  $\implies w \in S^{'}$. \\
  Number of neighbors in $N_k$ A.S. $\geq 1$, \\
  $|N_k| \geq \left(\frac{1}{2} n p \left(1-\omega^{-\frac{1}{3}}\right)\right)^k \approx c \cdot n$ \\
  $\implies \diam (G) = k + 1 $ A.S. (WHY?????)
  \qed
\end{pro}

\subsection{Scale free property}

Suppose $G$ is a random graph on $n$ vertices in the most general way - the set of edges is a random variable with any distribution. Let $p_n(k)$ be the random
variable denoting the expected proportion of vertices of degree $k$. Suppose $p(k) = \lim_{n\to \infty}p_n(k)$ exists for every $k$. Then we say that the distribution has the scale-free property if 
$$\lim_{k\to \infty}p(k)k^{\gamma} = C$$
...
\begin{itemize}
  \item add node $v$,
  \item add $m$ edges from $v$ (to $u$) in such way that $P(v \sim u) = \frac{\deg(u)}{\sum_x \deg(x)}$.
\end{itemize}
...
\begin{pro} \text{} \\
  ...
  Let $X_k$ be the number of vertices with degree $k$ when we have $n$ vertices and let $Y_k$ be the number of vertices who had degree $k$ at $n$ vertices and degree $k + 1$ at $n + 1$ vertices.
  By the law of total expected value we have 
  \begin{align*}
    E[Y_k] = E[Y_k | X_k = 0]&P(X_k = 0) + E[Y_k | X_k = 1]P(X_k = 1)\\ &+ \ldots + E[Y_k | X_k = n]P(X_k = n)  
  \end{align*}
  Under the assumption that $X_k = l$, we have $Y_k = I_1 + \ldots + I_l$ where $I_a = 1$ if the $a$-th vertex with degree $k$ is selected as the neighbor of the new vertex and $0$ otherwise.
  Then $$E[Y_k | X_k = l] = l \cdot E[I_a] = l \cdot P(I_a = 1) = l \cdot \frac{k}{\sum_u \deg(u)}.$$
  ...
  $p_{n+1}(k) \cdot (n+1) = p_n(k) \cdot n - p_n(k) \cdot \frac{nk}{2(n - m)} + p_n(k-1) \cdot \frac{n(k-1)}{2(n - m)}$, where \\
  $p_n(k) \cdot n$: degree $k \to k$, \\
  $p_n(k) \cdot \frac{nk}{2(n - m)}: k \to k+1$, \\
  $p_n(k-1) \cdot \frac{n(k-1)}{2(n - m)}: k-1\text{ or less} \to k$ (for $k > m$; for $k = m$ this summand is equal to $1$ because we add a vertex who had degree zero and now has degree $1$). \\
  Assuming without proof that $\lim_{n\to \infty}(p_{n+1}(k) - p_n(k))n = 0$ when sending $n \to \infty$ we get: \\
  ...
  Similarly to before, by sending $n \to \infty$ we get $p_m = -\frac{m}{2} \cdot p_m + 1$ and so:
  \begin{align*}
    &p_m = \frac{2}{m+2} \\
...
\begin{defn}[Transition matrix] \text{} \\
  $\Omega = \{x_1 \dots x_n\}$ \\
  $p_{ij} = P(X_{t+1} = x_j \mid X_t = x_i)$ \\
  $\begin{bmatrix}
    p_{11} & \dots & p_{1n} \\
    \vdots & & \vdots \\
    p_{n1} & \dots & p_{nn}
...
\begin{defn} \text{} \\
  \begin{itemize}
    ...
    \item $f_{ij}$: probability that $X_t = x_j$ for some $t$ assuming $X_0 = x_i$,
      % skica
    \item $h_{ij}$: expected number of steps needed to get to state $x_j$ starting in $x_i$ (hitting time),
    ...
    \item M.C. is aperiodic if there is no $c \in \{2, 3, 4 \dots\}$ such that all lengths of cycles in the transition graph are divisible by $c$.
  \end{itemize}
\end{defn}
\begin{theorem} \text{} \\
...
\begin{pro} \text{} \\
  ...
  Say we are at $i$-th step and we have selected $C_l = l_1 \vee l_2$ that is false where $l_1$ and $l_2$ are functions of $x_{l1}$ and $x_{l2}$. Let $\val(x_{l1})$ and $\val(x_{l2})$ be the values
  of $x_{l1}$ and $x_{l2}$ for which $l_1$ and $l_2$ are true. Then we must have $a_{l1} = \val(x_{l1})$ or $a_{l2} = \val(x_{l2})$
  because $C_l$ has to be true when evaluated in $a$. And because $C_l$ is false when evaluated in $b^i$ is false, then we must have both $b^i_{l1} \neq \val(x_{l1})$ and $b^i_{l2} \neq \val(x_{l2})$.
  This means that either $b^{i+1}_{l1} = \val(x_{l1})$ or $b^{i+1}_{l2} = \val(x_{l2})$. All of this together gives us for $ j \geq 1$
  $$P(\text{distance of } b^{i+1} \text{ to $a$ is } j-1 \mid \text{distance of $b^i$ to $a$ is } j) \geq \frac{1}{2}.$$

  We define an auxiliary Markov chain
  $$P = \begin{bmatrix}
    0 & 1 & 0 & 0 & \dots & \dots & 0 \\
    \frac{1}{2} & 0 & \frac{1}{2} & 0 & \dots & \dots & 0 \\
    0 & \frac{1}{2} & 0 & \frac{1}{2} & \dots & \dots & 0 \\
    \vdots & & & & & & \vdots \\
    0 & 0 & \dots & \dots & \frac{1}{2} & 0 & \frac{1}{2} \\
    0 & \dots & \dots & 0 & 0 & 1 & 0
  \end{bmatrix}.$$
  This is a finite irreducible Markov chain and so by theorem it has a unique stationary distribution
  $$\pi = (\frac{1}{2n}, \frac{1}{n} \dots \frac{1}{n}, \frac{1}{2n}).$$
  By theorem we know that $h_{i,i} = \frac{1}{\pi_i} = n$ for $i = 1, 2 \dots n-1$ and $h_{0,0} = h_{n,n} = 2n$. \\
  We also know that for every $i \in \{1, \ldots, n-1\}$ we have $n = h_{i,i} = 1 + \frac{1}{2} h_{i+1,i} + \frac{1}{2} h_{i-1,i}$. For every
  $i \in \{2, \ldots, n-2\}$ we have $h_{i+1,i} = h_{i-1,i}$ and so $h_{i+1,i} = n - 1$. We know that $h_{n, n - 1} = 1$ and $2n = h_{0,0} = 1 + h_{1,0}$,
  so $h_{1,0} = 2n - 1$. Thus for every $i \leq n - 1$ we have $h_{i+1,i} \leq 2n$ and so
  $$h_{n,0} \leq h_{n,n-1} + \dots + h_{1,0} \leq 2n^2.$$
  We could show that because
  $$P(\text{distance of } b^{i+1} \text{ to $a$ is } j-1 \mid \text{distance of $b^i$ to $a$ is } j) \geq \frac{1}{2},$$
  the expected number of steps to reduce the Hamming distance from $n$ to $0$ is less than $h_{n,0}$. This proves that
  $$E(\text{steps in algorithm to reach correct solution}) = E(Z) \leq 2n^2$$
  and so
  \begin{align*}
    P(\text{algorithm }&\text{hasn't reached correct solution after $8n^2$ steps}) \\&= P(Z > 8n^2) \stackrel{\text{Markov}}{\leq} \frac{E(Z)}{8n^2} \leq \frac{1}{4}.
  \end{align*}  
  \qed
\end{pro}
...
Suppose we have a set $\Omega$ from which we want to sample uniformly. If the set is small, we can just explicitly list all elements
and select one at random. But if we have a very large set we can't do that. Here we develop an approach to an approximation of 
uniform sampling in such large sets.

Let $\Omega$ be a set and let $G$ be a undirected graph with vertices on $\Omega$. We form M. C: 
$$P_{x,y} = \begin{cases}
  \frac{1}{M}: &\text{ if } x \neq y \land x \sim y \\
  0: &\text{ if } x \neq y \land x \nsim y \\
  1 - \frac{|N(x)|}{M}: &\text{ if } x = y
\end{cases}$$
where we choose some $M \geq \max_{v \in \Omega} |N(v)|$. \\
If $G$ is connected, then M.C. is irrecudible. \\
Let's prove that $\pi = (\frac{1}{|\Omega|} \dots \frac{1}{|\Omega|})$ is a stationary distribution of $P$.
\begin{align*}
  (\pi P)_x &= \sum_y \pi_y P_{y,x} \\
  &= \sum_{y \in N(x)} \frac{1}{M} \cdot \frac{1}{|\Omega|} + \frac{1}{|\Omega|}
    \left(1 - \frac{|N(x)|}{M}\right) = \frac{1}{|\Omega|} = \pi_x.
\end{align*}
This means that if we walk on the Markov chain long enough, we end up in state $x$ with probability approximately $\pi_x = \frac{1}{|\Omega|}$.
This gives us an approximation of uniform sampling.
\begin{ex} \text{} \\
  Let $G = (V, E)$ be a graph, finding largest independent set $(\forall u,v: u \nsim v)$ is NP-complete. Let's try sampling a uniformly random independent set-
  Let $\Omega = \{\text{independent sets}\}$ and let $\Gamma$ be a graph with $\Omega$ for vertices where to independent sets are connected
  if they differ by exactly one vertex. Graph $\Gamma$ is obviously connected because for every independent sets $A$ and $B$ we have a walk where we
  delete vertex by vertex from $A$ and then add vertex by vertex from $B$. We define a random walk on $\Gamma$ as follows:\\
  Let $X_0$ be an arbitrary independent set. Suppose we have $X_i = U$. Then we form $X_{i+1}$ as follows:
  \begin{itemize}
    ...
    \item else $X_{i+1} = U$.
  \end{itemize}
  In $\Gamma$ independent set $U$ is adjacent only to vertices of form to $U \cup \{v\}$ and $U \textbackslash \{v\}$. The walk will go from $U$
  to one of these neighbors only if we select $v$. Because we select a vertex at random, the probability of that happening is $\frac{1}{|V|}$.
  The process will never go from $U$ to any independent set not adjacent to $U$ and the probability of the process staying at $U$ is precisely
  $1 - \frac{N(U)}{|V|}$. Thus if we select $M = |V|$, this walk is precisely the Markov chain $P$ we defined above, meaning for every $U \in \Omega$
  we have
  $$\lim_{t \to \infty} P(X_t = u) = \frac{1}{|\Omega|}$$
  which gives us an approximation of uniform sampling of independent sets.
\end{ex}


\section{Metropolis algorithm}

Suppose we have a set $\Omega$ and we want to sample elements with a chosen distribution $\pi$ from $\Omega$. We do so by constructing 
a connected graph $G$ with vertices on $\Omega$ and defining a random walk on this graph which corresponds to an irreducible Markov chain
with stationary distribution $\pi$. Then by walking around $G$ for long enough time, we get an approximation of sampling with distribution $\pi$.
Let's define the following Markov chain:
$$P_{x,y} = \begin{cases}
  \frac{1}{M} \cdot \min \left(1, \frac{\pi_y}{\pi_x}\right): &\text{ if } x \neq y \land x \sim y \\
  0: &\text{ if } x \neq y \land x \nsim y \\
  1 - \sum_{z \in N(x)}\frac{1}{M} \cdot \min \left(1, \frac{\pi_z}{\pi_x}\right): &\text{ if } x = y
\end{cases}$$
where we choose some $M \geq \max_{v \in \Omega} |N(v)|$. Now let's prove that $\pi$ is indeed the stationary distribution of this irreducible
Markov chain:
\begin{align*}
  (\pi P)_x &= \sum_y \pi_y P_{y,x} =
    \sum_{y \in N(x)} \pi_y \frac{1}{M} \min \left(\left(1, \frac{\pi_x}{\pi_y}\right)\right)
    + \pi_x \left(1 - \sum_{y \in N(x)} \frac{1}{M} \min \left(1, \frac{\pi_y}{\pi_x}\right)\right) \\
  &= \sum_{y \in N(x), \pi_y < \pi_x} \pi_y \frac{1}{M} \cdot 1 +
    \sum_{y \in N(x), \pi_y \geq \pi_x} \pi_y \frac{1}{M} \frac{\pi_x}{\pi_y} + \pi_x \\
  &- \sum_{y \in N(x), \pi_y < \pi_x} \pi_x \frac{1}{M} \frac{\pi_y}{\pi_x} -
    \sum_{y \in N(x), \pi_y \geq \pi_x} \pi_x \frac{1}{M} \cdot 1 \\
  &= \pi_x.
\end{align*}
\begin{ex} \text{} \\
  $\Omega = \Z \cap [-1000,1000]$ \\
  $\pi \sim e^{-\frac{(x-\mu)^2}{2 \delta}}$ \\
  We construct a graph $G$ where two integers are adjacent if they differ by $1$. Then the neighborhood of each
  integer contains at most $2$ elements and so we can choose $M = 2$. The graph is obviously connected. The following
  process moves from current vertex $x$ to an adjacent vertex $y$ with probability $\frac{1}{2} \min \left(1, \frac{\pi_y}{\pi_x}\right)$,
  it never moves to a non-adjacent vertex and it remains put with probability $1 - \sum_{z \in N(x)}\frac{1}{M} \cdot \min \left(1, \frac{\pi_z}{\pi_x}\right)$
  which we can quickly prove with law of total probability. Thus the process is a special case of the above described
  Markov chain and distribution of the following process converges towards $\pi$.
  \\~\\
  \begin{lstlisting}
    $X_0$ arbitrary
    for $i$ = in range(1,$m$):
      $y$ <- uniformly from $\{X_i-1,X_i+1\}$
...
\end{ex}
\begin{ex} \text{} \\
  Finding a maximum of a positive function $f$ defined on $\Omega$ can be approximated by using the metropolis algorithm to sample proportionally 
  to $f$ from $\Omega$. We sample using the distribution $\pi(x) = \frac{f(x)}{Z}$ where $Z = \sum_{x \in \Omega}f(x)$. However
  we don't have to calculate $Z$ because metropolis algorithm only uses ratios $\frac{\pi(y)}{\pi(x)} = \frac{f(y)}{f(x)}$
  which is useful because $Z$ could be very hard to calculate.
\end{ex}
